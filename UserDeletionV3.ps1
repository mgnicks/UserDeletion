...
$StartDate = read-host "Please provide the start date for the Out Of Office (format MM/DD/YYYY HH:MM:SS)"
$EndDate = read-host "Please provide the end date for the Out Of Office (format MM/DD/YYYY HH:MM:SS)"
...
set-MailboxAutoReplyConfiguration -Identity $mailboxToConvert -AutoReplyState Scheduled -StartTime $StartDate -EndTime $EndDate -InternalMessage $IntMessage -ExternalMessage $ExtMessage
...
echo "mailbox already being forwarded to $IsForwarderSet" >> $logfile
...
function Show-Menu {
    ...
    Write-Host "2: Press '2' to disable multiple accounts using a csv file (opens a window to choose the CSV file)"
